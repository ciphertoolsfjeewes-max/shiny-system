...
# --- КОНФИГУРАЦИЯ ---
API_ID = os.getenv("24366454")  # Исправлено
API_HASH = os.getenv("2389220c820dbc00519fddf14076b049")  # Исправлено
GEMINI_API_KEY = os.getenv("AIzaSyCb6P9bUqopRtW8NFX2_gTY-gyYYPd-gwU")  # Исправлено

# Проверка наличия ключей API
if not all([API_ID, API_HASH, GEMINI_API_KEY]):
    logger.critical("Отсутствуют переменные окружения TELEGRAM_API_ID, TELEGRAM_API_HASH или GEMINI_API_KEY. Проверьте ваш .env файл.")
    raise ValueError("Не удалось получить необходимые переменные окружения.")
